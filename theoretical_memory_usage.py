...
def compute_activation_memory(args, num_microbatches, verbose=False):
    ...
    activation_memory += (
        8 * args.seq_length * args.micro_batch_size * args.pipeline_model_parallel
    )
    ...
    if args.pipeline_model_parallel is not None:
        interleaved_schedule_memory_penalty = 1 + (
            (args.pipeline_model_parallel - 1)
            / (args.pipeline_model_parallel * args.pipeline_model_parallel)
        )
        in_flight_microbatches = math.ceil(
            interleaved_schedule_memory_penalty * args.pipeline_model_parallel
        )
        if verbose:
            print(
